...
const outputDir = path.join(process.cwd(), 'output', 'playwright', 'virtual-storage-submit-proof', runId);
...
async function loginViaApi() {
  ...
  const payload = await parseJson(response, 'user login');
  const rawUser = payload?.data?.user ?? payload?.user;
  const token = payload?.data?.token ?? payload?.token;
  if (!token || !rawUser?.id) throw new Error('Login did not return a usable token');
  const fullName = [rawUser.first_name, rawUser.last_name].filter(Boolean).join(' ').trim() || rawUser.email || 'user';
  return {
    token,
    storedUser: {
      id: String(rawUser.id),
      email: String(rawUser.email || credentials.user.email),
      name: fullName,
...
async function apiJson(pathname, session, options = {}) {
  const response = await fetch(`${target.coreUrl}${pathname}`, {
    ...options,
    headers: {
...
async function main() {
  ...
  const fileName = `qa-virtual-storage-submit-${stamp}.pdf`;
  ...
  const result = {
    ...
    fileName,
    uploadedDocumentId: '',
    uiStatusVisible: false,
    uiStoredDocumentVisible: false,
  ...
  try {
    ...
    await page.getByText('Virtual Storage', { exact: false }).first().waitFor({ timeout: 30000 });
    await page.locator('#virtual-storage-category').selectOption('template:supporting');
    await page.locator('#virtual-storage-file').setInputFiles(filePath);
    await page.getByRole('button', { name: /^Upload$/ }).click();

    await page.getByText('Document uploaded to Virtual Storage.', { exact: false }).waitFor({ timeout: 60000 });
    result.uiStatusVisible = true;
    await page.getByText(fileName, { exact: false }).first().waitFor({ timeout: 30000 });
    result.uiStoredDocumentVisible = true;
    result.screenshotAfterUpload = path.join(outputDir, 'virtual-storage-after-upload.png');
    await page.screenshot({ path: result.screenshotAfterUpload, fullPage: true });

    const documentsPayload = await apiJson('/api/v1/virtual-storage/documents', session);
    const documents = documentsPayload?.data?.documents || documentsPayload?.documents || [];
    const uploaded = documents.find((document) => document.file_name === fileName);
    if (!uploaded?.id) {
      throw new Error('Uploaded QA document was not returned by the Virtual Storage API.');
    }
    result.uploadedDocumentId = uploaded.id;
    result.apiDocumentVisible = true;

    try {
      await apiJson(`/api/v1/documents/${encodeURIComponent(uploaded.id)}`, session, { method: 'DELETE' });
      result.cleanupStatus = 'deleted';
      await page.reload({ waitUntil: 'domcontentloaded', timeout: 120000 });
      await page.getByText('Virtual Storage', { exact: false }).first().waitFor({ timeout: 30000 });
      const remaining = await page.getByText(fileName, { exact: false }).count();
      if (remaining > 0) {
        throw new Error('Deleted QA document still appeared in the UI after reload.');
      }
      result.screenshotAfterCleanup = path.join(outputDir, 'virtual-storage-after-cleanup.png');
      await page.screenshot({ path: result.screenshotAfterCleanup, fullPage: true });
    } catch (cleanupError) {
      result.cleanupStatus = `failed: ${cleanupError.message}`;
      throw cleanupError;
    }
  ...
  const outputPath = path.join(outputDir, 'virtual-storage-submit-proof.json');
...
main().catch((error) => {
  ensureDir(outputDir);
  const outputPath = path.join(outputDir, 'virtual-storage-submit-proof.error.json');
