...
const outputDir = path.join(process.cwd(), 'output', 'playwright', 'case-file-submit-proof', runId);
...
async function collectFastTrackCaseIds(page) {
  await page.goto(`${target.baseUrl}/user/dashboard/fast-track`, { waitUntil: 'domcontentloaded', timeout: 120000 });
  ...
  for (let index = 0; index < Math.min(count, 12); index += 1) {
    ...
    if (caseId && !caseIds.includes(caseId)) caseIds.push(caseId);
  }
  return caseIds;
}
...
async function main() {
  ...
  const fileName = `qa-case-file-submit-${stamp}.pdf`;
  ...
  try {
    const caseIds = await collectFastTrackCaseIds(page);
    for (const caseId of caseIds) {
      await page.goto(`${target.baseUrl}/user/dashboard/case-file?case=${encodeURIComponent(caseId)}&tab=documents&section=documents`, {
      ...
      try {
        ...
        result.screenshotAfterUpload = path.join(outputDir, 'case-file-after-upload.png');
        await page.screenshot({ path: result.screenshotAfterUpload, fullPage: true });
        break;
      } catch (error) {
        if (caseId === caseIds[caseIds.length - 1]) throw error;
      }
    }

    if (!result.caseId) throw new Error('No accessible case file with enabled upload was found.');
    const uploaded = await findCaseDocument(result.caseId, fileName, session);
    const uploadedDocumentId = uploaded.document_id || uploaded.document?.id || uploaded.id;
    if (!uploadedDocumentId) throw new Error('Uploaded case-file QA document was returned by API without a document id.');
    result.uploadedDocumentId = uploadedDocumentId;
    result.apiCaseFileDocumentVisible = true;

    try {
      await apiJson(target.coreUrl, `/api/v1/documents/${encodeURIComponent(uploadedDocumentId)}`, session, { method: 'DELETE' });
      result.cleanupStatus = 'deleted';
      await page.reload({ waitUntil: 'domcontentloaded', timeout: 120000 });
      await page.getByText('Upload all documents', { exact: false }).first().waitFor({ timeout: 30000 });
      const remaining = await page.getByText(fileName, { exact: false }).count();
      if (remaining > 0) throw new Error('Deleted QA case-file document still appeared in UI after reload.');
      const stillInApi = await findCaseDocument(result.caseId, fileName, session);
      if (stillInApi) throw new Error('Deleted QA case-file document still appeared in API after cleanup.');
      result.screenshotAfterCleanup = path.join(outputDir, 'case-file-after-cleanup.png');
  ...
  const outputPath = path.join(outputDir, 'case-file-submit-proof.json');
...
main().catch((error) => {
  ensureDir(outputDir);
  const outputPath = path.join(outputDir, 'case-file-submit-proof.error.json');
