...
test('manager dashboard KPI counters expose destination actions', () => {
  ...
  assert.match(source, /title="Active Fast Track cases"[\s\S]*?onClick=\{\(\) => navigate\('\/manager\/fast-track'\)\}/);
  ...
  assert.match(source, /No active cases\. Live queue status is shown in the response tracker below\./);
  assert.match(source, /const \[dashboardMetricsLoading, setDashboardMetricsLoading\] = useState\(true\)/);
  assert.match(source, /title="Active Fast Track cases"[\s\S]*?loading=\{dashboardMetricsLoading\}/);
