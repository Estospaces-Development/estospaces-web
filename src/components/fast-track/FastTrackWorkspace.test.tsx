...
test("admin fast-track workspace constrains compact layouts inside the viewport", () => {
  ...
  assert.match(source, /min-w-0 max-w-full space-y-3 overflow-x-hidden pb-16 sm:space-y-6/);
  assert.match(source, /grid min-w-0 max-w-full gap-4/);
  assert.match(source, /min-w-0 max-w-full space-y-3 sm:space-y-6/);
  assert.match(source, /min-w-0 max-w-full overflow-hidden rounded-2xl[^\"]*sm:rounded-\[26px\]/);
});
