        ...
        return (
            <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Application Withdrawn</h3>
                        <p className="text-gray-500 dark:text-gray-400">You have withdrawn this application</p>
                    </div>
                </div>
            </div>
        );
                        ...
                        return (
                            <div key={stage.id} className="relative flex items-start pb-8 last:pb-0">
                                {index < stages.length - 1 && (
                                    <div className={`absolute left-6 top-12 w-0.5 h-full -ml-px ${isCompleted ? 'bg-green-500' : isCurrent && !isRejected ? 'bg-orange-300' : 'bg-gray-200 dark:bg-gray-700'}`} />
                                )}
                                ...
                                <div className="ml-4 flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <h4 className={`font-semibold ${isCompleted || isCurrent ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>{stage.label}</h4>
                                    ...
                                    <p className={`text-sm mt-1 ${isCompleted || isCurrent ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-600'}`}>{stage.description}</p>
                                    {isCurrent && status === APPLICATION_STATUS.DOCUMENTS_REQUESTED && (
                                        <div className="mt-3 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
                                            ...
                                            <p className="text-sm text-orange-600 dark:text-orange-300 mt-1">Please upload the requested documents to proceed</p>
                                        </div>
                                    )}
                                </div>
                            </div>
