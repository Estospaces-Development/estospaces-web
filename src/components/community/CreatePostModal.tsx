...
const CreatePostModal: React.FC<CreatePostModalProps> = ({ isOpen, onClose, onSubmit }) => {
    ...
    const isFormReady = title.trim().length > 0 && content.trim().length > 0;
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] overflow-y-auto bg-black/50 p-3 backdrop-blur-sm animate-in fade-in duration-200 sm:p-4">
            ...
            <div className="flex max-h-[calc(100vh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl animate-in zoom-in-95 duration-200 dark:border-zinc-800 dark:bg-zinc-900">
                {/* Header */}
                ...
                <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
                    ...
                    <div className="flex shrink-0 flex-col-reverse gap-3 border-t border-gray-100 p-4 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-end">
                        ...
                        <button type="submit" disabled={!isFormReady} className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 disabled:cursor-not-allowed text-white font-medium py-3 px-6 rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg">
                            Post to Community
                        </button>
                    </div>
                </form>
