...
export default function AddPropertyPage() {
    ...
    const loadPropertyForEdit = async () => {
      ...
      const loadedFormData: FormData = {
        ...
        preferredContactMethod:
          property.contact?.preferredContactMethod || "any",
  ...
  const syncFieldErrors = useCallback(
    (nextData: FormData, fields: string[]) => {
      if (fields.length === 0) {
        return;
      }

      const validationValues = getValidationValues(nextData);
      setErrors((prev) => {
  ...
  const validateStep = (step: number): boolean => {
    const newErrors = validateManagerPropertyStep(
      step,
  ...
  const getFieldsToRevalidate = (field: keyof FormData): string[] => {
    switch (field) {
      case "totalArea":
        return ["totalArea", "carpetArea"];
      case "carpetArea":
        return ["carpetArea", "totalArea"];
      case "floorNumber":
        return ["floorNumber", "totalFloors"];
      case "totalFloors":
        return ["totalFloors", "floorNumber"];
      case "listingType":
        return ["listingType", "minimumLease"];
      default:
        return [String(field)];
    }
  };
  ...
  const handleInputChange = <K extends keyof FormData>(
    ...
    setIsDirty(true);
    syncFieldErrors(nextData, getFieldsToRevalidate(field));
  };
  ...
  const toggleAmenity = (
    category: keyof typeof amenitiesGroups,
    amenityId: string,
  ) => {
    setFormData((prev) => {
  ...
  const removeImage = (index: number) => {
    ...
    setFormData(nextData);
    setIsDirty(true);
    const nextPreviewCount = Math.max(imagePreviews.length - 1, 0);
    setErrors((prev) => {
      const nextErrors = { ...prev };
      if (nextData.images.length > 0 || nextPreviewCount > 0) {
        delete nextErrors.images;
      } else {
    ...
    if (newFiles.length > 0) {
      try {
        ...
        if (uploadedUrls.length !== newFiles.length) {
          throw new Error(
            "Image upload did not complete for every selected file.",
          );
        }
      } catch (err) {
        throw err;
  ...
  const processVideos = async (files: (File | string)[]): Promise<string[]> => {
    ...
    if (newFiles.length > 0) {
      try {
        ...
        if (uploadedUrls.length !== newFiles.length) {
          throw new Error(
            "Video upload did not complete for every selected file.",
          );
        }
      } catch (err: any) {
        throw err;
  ...
  const buildPropertyData = async (): Promise<Partial<Property>> => {
    ...
    const availableFrom = formData.availableFrom.trim() || undefined;
    const minimumLease =
  ...
  const finalizeMediaEntity = async (
    propertyId: string,
  ): Promise<string | null> => {
    if (idValue) {
      return null;
  ...
  const handleSaveOrPublish = async () => {
    ...
    if (isSubmissionAction && managerVerificationLoading) {
      showToast(
        "Checking your manager verification status. Please try again in a moment.",
        "info",
      );
      return;
    }
    ...
    try {
      ...
      if (mode === "edit" && idValue) {
        const nextStatus = isEditSubmission
          ? "pending_approval"
          : formData.status;
        const result = await updateProperty(
          idValue,
      ...
      showToast(
        `Failed to ${actionWord} property: ${error?.message || "Unknown error"}`,
        "error",
      );
    } finally {
      setSaving(false);
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between overflow-x-auto pb-2">
            {steps.map((step, index) => {
              ...
              const isAccessible =
                mode === "edit" ? true : isCompleted || isCurrentStep;
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center flex-1 min-w-0">
                    <button
                      onClick={() =>
                        isAccessible && handleTabClick(step.number)
                      }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {listingTypes.map((type) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleInputChange("listingType", type.value)
                        }
                ...
                <div>
                  ...
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset ${reviewStatusBadge.badgeClassName}`}
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${reviewStatusBadge.dotClassName}`}
                      />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={getFieldClassName("facing")}
                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {furnishingOptions.map((option) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleInputChange("furnishing", option.value)
                        }
                        className={`px-4 py-2 rounded-lg border transition-all ${
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleInputChange(
                        "condition",
                        e.target.value as PropertyCondition,
                      )
                    }
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-gray-900 dark:text-white"
                  >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {amenities.map((amenity) => (
                      <button
                        ...
                        onClick={() =>
                          toggleAmenity(
                            category as keyof typeof amenitiesGroups,
                            amenity.id,
                          )
                        }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={getFieldClassName("alternatePhone")}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={getFieldClassName("inclusions")}
                    placeholder="e.g., Appliances, furniture, utilities"
                  />
                  {renderFieldError("inclusions")}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={getFieldClassName("exclusions")}
                    placeholder="e.g., Parking fees, utility bills"
                  />
                  {renderFieldError("exclusions")}
                </div>
              </div>
            </div>
